...
function updateScenarioControls(scenarioData) {
    ...
    // Frame color
    var cardInfo = scenarioContainer.closest('.card').querySelector('.card-icon');
    var parentColor = cardInfo.style.backgroundColor;
    if (parentColor === '') {
        parentColor = window.getComputedStyle(cardInfo).backgroundColor;
    }
    scenarioContainer.getElementsByClassName('div_scenario')[0].style.backgroundColor = parentColor.replace(')', ', 0.2)').replace('rgb', 'rgba');
    // Status Label
